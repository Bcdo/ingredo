...
House pattern: expected outcomes as `ServiceResult` (`NotFound` → 404 unknown code, `Conflict` → 409 own-household/sole-member), validation 400s, everything else the global middleware. No information leak: an unknown join code is indistinguishable from a nonexistent one (both 404; codes are not practically enumerable — 30⁶ ≈ 7×10⁸ space, and the join endpoint sits behind authentication).
