...
5. **Leave sheds one membership — the ACTIVE one.** `LeaveAsync`: remove the membership named by the token claim; owner promotion for the remaining members as today. When the departing user was the household's LAST member: the household (and its server content) is deleted — but ONLY when the user has another membership to land on; a sole member with no other membership gets `Conflict` exactly as today (leaving would destroy content just to mint an identical empty household — and the leave dialog promises content survives). Response: `AuthResponse` with the user's oldest REMAINING membership active; if the shed membership was their last (others stayed behind in the household), a fresh personal household is created first (today's behavior).
